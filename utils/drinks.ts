...
export const getDrinkOption = (typeKey: DrinkTypeKey, optionKey: DrinkOptionKey): DrinkOption | undefined => {
  return DRINK_TYPES[typeKey]?.options.find(option => option.key === optionKey);
};

// Get default drink option for a type
export const getDefaultDrinkOption = (typeKey: DrinkTypeKey): DrinkOption | undefined => {
...
export const formatDrinkOption = (
  ...
  if (unit === 'ml' && preferredUnit === 'oz') {
    const ozAmount = (amount * 0.033814).toFixed(1);
    return `${optionName} (${ozAmount}oz)`;
  }
...
export const calculateTotalAlcohol = (drinks: DrinkLog[]): DrinkCalculation => {
  ...
  drinks.forEach(drink => {
    const alcoholPercentage = drink.alcohol_percentage || DRINK_TYPES[drink.drink_type].alcohol_percentage || 5;
...
export const validateDrinkLog = (drinkLog: Partial<DrinkLog>): { valid: boolean; errors: string[] } => {
  const errors: string[] = [];
  ...
  if (drinkLog.amount_ml !== undefined && (isNaN(drinkLog.amount_ml) || drinkLog.amount_ml <= 0)) {
    errors.push('Amount must be a positive number');
  }
  
  if (drinkLog.alcohol_percentage !== undefined && (isNaN(drinkLog.alcohol_percentage) || drinkLog.alcohol_percentage < 0 || drinkLog.alcohol_percentage > 100)) {
