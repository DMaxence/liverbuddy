...
export const calculateAlcoholProcessingRate = (profile: UserProfile = DEFAULT_USER_PROFILE): number => {
  ...
  baseRate *= fitnessMultipliers[profile.fitness_level];
...
export const calculateEstimatedBAC = (
  ...
  drinks.forEach(drink => {
    ...
    const hoursElapsed = Math.max(0, (timePoint.getTime() - drinkTime.getTime()) / (1000 * 60 * 60));
    
    const alcoholInDrink = calculateAlcoholUnits(drink) * 10;
    const alcoholRemaining = Math.max(0, alcoholInDrink - (hoursElapsed * processingRate));
  ...
  const bacPercentage = (totalAlcoholGrams / (profile.weight_kg * 1000 * r)) * 100;
...
export const assessMedicalRisk = (
  ...
  if (dailyUnits >= 8 || weeklyUnits >= 35 || (bingeDays >= 2 && consecutiveDays >= 3)) {
    return 'high';
  }
  
  // Moderate risk indicators
  if (dailyUnits >= 6 || weeklyUnits >= 21 || bingeDays >= 1 || consecutiveDays >= 5) {
    return 'moderate';
  }
...
export const calculateMedicalHealthScore = (
  ...
  // Check if this is a recovery day (no drinks today)
  if (units === 0) {
    const recoveryScore = calculateLiverRecoveryScore(allLogs, targetDate, profile);
    if (recoveryScore > 0) {
  ...
  const weeklyUnits = weekLogs.reduce((sum, log) => sum + calculateAlcoholUnits(log), 0);
  
  console.log(`Weekly units: ${weeklyUnits}`);
  
  // Count binge days in past week (6+ units in a day)
  const bingeDays = Array.from(new Set(weekLogs.map(log => {
    ...
    const dayConsumption = calculateDailyConsumption(allLogs, logDateString);
  ...
  const risk = assessMedicalRisk(units, weeklyUnits, bingeDays, consecutiveDays);
  const riskScores = {
    'very_low': 90,
    'low': 75,
    'moderate': 55,
  ...
  if (dayLogs.length >= 2) {
    ...
    const drinkingDurationHours = (new Date(sortedLogs[sortedLogs.length - 1].timestamp).getTime() - 
                                  new Date(sortedLogs[0].timestamp).getTime()) / (1000 * 60 * 60);
  ...
  let bacPenalty = 0;
  if (dayLogs.length > 0) {
    const peakBAC = Math.max(...dayLogs.map(log => 
      calculateEstimatedBAC(dayLogs.filter(l => new Date(l.timestamp) <= new Date(log.timestamp)), new Date(log.timestamp), profile)
    ));
    
    if (peakBAC >= 0.15) {
      bacPenalty = 20; // Dangerous BAC level
    } else if (peakBAC >= 0.10) {
...
export const getDefaultUserProfile = (): UserProfile => ({ ...DEFAULT_USER_PROFILE });
